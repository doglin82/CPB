...
<?php 
 ...
 $blog_masonry_col=ot_get_option('blog_masonry_columns'); if($blog_masonry_col==''){$blog_masonry_col=3;}

      $out_title=get_the_title(get_option('page_for_posts', true));
	  $front= is_home(); $GLOBALS['front'] = $front;
...
<?php } ?>
				...
				$postid=get_the_id();
				$blog_style=get_post_format($postid); if($blog_style!='video' && $blog_style!='$blog_style' && $blog_style!='default' || $blog_style==""){$blog_style="default";}
				...
				if(!is_sticky()){
				 ...
				 get_template_part( 'content-masonry-'.$blog_style, $format );
				 }else{
				 get_template_part( 'content-'.$blog_style, $format );	
				 }
				}
				}else{
					$postid=get_the_id();
