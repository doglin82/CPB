...
<a class="post_img_container" href="<?php the_permalink(); ?>">
<?php $infos=array();	if(ot_get_option('posts_fields','')) { $infos=ot_get_option('posts_fields','');}?>
	...
	<?php 
		...
		$vwidth = get_post_meta(get_the_ID(), 'dbt_post_video_width', false,true);
		$vheight = get_post_meta(get_the_ID(), 'dbt_post_video_height', false,true);
		$vvwidth="100%";$vvheight=350;if(isset($vwidth[0]))$vvwidth=$vwidth[0];if(isset($vheight[0]))$vvheight=$vheight[0];
		$vids='';if(!isset($vwidth[0]))$vids='vids';
		if (isset($ids[0]) && $ids[0] != '') 
		{
			if ($videotype[0] == "vimeo" || $videotype == "vimeo"){echo '<iframe src="http://player.vimeo.com/video/' . $ids[0] . '" width="'.$vvwidth.'" height="'.$vvheight.'"  class="post-slider-border  generalframe '.$vids.'"></iframe>'; }
		else {
			echo '<iframe src="http://www.youtube.com/embed/' . $ids[0] . '?wmode=transparent" width="'.$vvwidth.'" height="'.$vvheight.'" class="post-slider-border  generalframe '.$vids.'"></iframe>';
		   }
		}
		echo '</div>';	
		}
